...
BENCHMARK_NAME = "graph_canonicalization"

DEFAULT_OUTPUT_DIR = RESULTS_DIR / BENCHMARK_NAME
DEFAULT_OUTPUT_NAME = "graph_canonicalization_result.json"
...
def canonicalize_graph(fgraph: FunctionGraph) -> FunctionGraph:
    """
    Canonicalize the given function graph using PyTensor canonicalization rewrites.
    """

    canonicalizer = _ptmode.optdb.query("+canonicalize")
    canonicalizer.rewrite(fgraph)

    return fgraph


def validate_canonicalized_graph(
    fgraph: FunctionGraph,
    n_apply_nodes_before: int,
) -> dict[str, Any]:
    """
    Validate the canonicalized function graph and return a summary of its properties.
    """
    ...
    if n_apply_nodes_after == 0:
        raise ValueError("Canonicalized graph has no apply nodes")
...
def measure_graph_canonicalization_memory(
    workspace_path: Path,
    target: str,
    mode: str,
) -> tuple[FunctionGraph, dict[str, float | int]]:
    """
    Measure the memory usage of canonicalizing the graph for the given workspace,
    target, and mode.
    """
    ...
    canonicalized_fgraph = canonicalize_graph(fgraph)

    current_rss_after_mb = get_current_rss_mb()
    peak_rss_after_mb = get_peak_rss_mb()

    return canonicalized_fgraph, {
        "memory_n_runs": 1,
...
def measure_graph_canonicalization_timing(
    workspace_path: Path,
    target: str,
    mode: str,
    n_runs: int,
    ...
    """
    Measure the time taken to canonicalize the graph for the given workspace,
    target, and mode.
    """
...
def run_single_benchmark(
    ...
    """
    Run a single graph canonicalization benchmark for the given workspace,
    target, and mode.
    """

    fgraph, memory_summary = measure_graph_canonicalization_memory(
        workspace_path=workspace_path,
        target=target,
        mode=mode,
    )

    validation_summary = validate_canonicalized_graph(
        fgraph=fgraph,
    ...
    timings = measure_graph_canonicalization_timing(
        workspace_path=workspace_path,
        target=target,
        mode=mode,
        n_runs=n_runs,
...
def print_result(result: dict[str, Any]) -> None:
    ...
    print()
    print("=" * 72)
    print("Graph canonicalization benchmark")
    print("=" * 72)
...
def make_plots(results: list[dict[str, Any]], plot_dir: Path) -> None:
    ...
    make_bar_plot(
        results=results,
        output_path=plot_dir / "graph_canonicalization_wall_time.png",
        title="Graph canonicalization wall time",
    ...
    if should_plot_metric(results, "current_rss_delta_mb"):
        make_bar_plot(
            results=results,
            output_path=plot_dir / "graph_canonicalization_current_rss_delta.png",
            title="Graph canonicalization current RSS delta",
    ...
    if should_plot_metric(results, "peak_rss_delta_mb"):
        make_bar_plot(
            results=results,
            output_path=plot_dir / "graph_canonicalization_peak_rss_delta.png",
            title="Graph canonicalization peak RSS delta",
