...
def validate_canonicalized_graph(
    ...
    """Validate the canonicalized function graph and return a summary."""

    if fgraph is None:
        raise ValueError("Canonicalized graph is None")
...
def make_error_result(
    ...
    """
    Build a structured benchmark result for a failed graph canonicalization run.
    """
...
def print_error_result(result: dict[str, Any]) -> None:
    ...
    print()
    print("=" * 72)
    print("Graph canonicalization benchmark FAILED")
    print("=" * 72)
    print(f"Workspace: {result['workspace']}")
    print(f"Target:    {result['target']}")
    print(f"Mode:      {result['mode']}")
