...
from src import run_graph_canonicalization as benchmark
...
def valid_result() -> dict[str, Any]:
    return {
        "benchmark": "graph_canonicalization",
        ...
        "apply_node_delta": -1,
        "rewriter": "canonicalize",
    }


@pytest.fixture
def second_valid_result(valid_result: dict[str, Any]) -> dict[str, Any]:
...
def test_canonicalize_graph_success(
    ...
    class FakeOptDB:
        def query(self, query: str) -> FakeCanonicalizer:
            assert query == "+canonicalize"
...
def test_measure_graph_canonicalization_memory_success(
    ...
    monkeypatch.setattr(benchmark, "canonicalize_graph", lambda fgraph: fgraph)
    ...
    fgraph, summary = benchmark.measure_graph_canonicalization_memory(
...
def test_measure_graph_canonicalization_memory_propagates_build_error(
    monkeypatch: pytest.MonkeyPatch,
    workspace_path: Path,
) -> None:
    ...
    with pytest.raises(RuntimeError, match="build failed"):
        benchmark.measure_graph_canonicalization_memory(
            workspace_path, "L_ch0", "FAST_RUN"
        )


def test_measure_graph_canonicalization_memory_propagates_canonicalize_error(
    monkeypatch: pytest.MonkeyPatch,
    fake_fgraph: SimpleNamespace,
    workspace_path: Path,
) -> None:
    ...
    def failing_canonicalize_graph(fgraph: Any) -> None:
        raise RuntimeError("canonicalize failed")

    monkeypatch.setattr(benchmark, "canonicalize_graph", failing_canonicalize_graph)
...
def test_measure_graph_canonicalization_timing_success(
    monkeypatch: pytest.MonkeyPatch,
    fake_fgraph: SimpleNamespace,
    workspace_path: Path,
) -> None:
    ...
    def fake_canonicalize_graph(fgraph: Any) -> Any:
        canonicalize_calls.append(fgraph)
        return fgraph

    monkeypatch.setattr(benchmark, "build_log_prob", fake_build_log_prob)
    monkeypatch.setattr(benchmark, "build_function_graph", lambda log_prob: fake_fgraph)
    monkeypatch.setattr(benchmark, "canonicalize_graph", fake_canonicalize_graph)
    monkeypatch.setattr(benchmark.time, "perf_counter", lambda: next(perf_values))

    timings = benchmark.measure_graph_canonicalization_timing(
        workspace_path=workspace_path,
        target="L_ch0",
        mode="FAST_RUN",
        n_runs=3,
    ...
    assert canonicalize_calls == [fake_fgraph, fake_fgraph, fake_fgraph]


def test_measure_graph_canonicalization_timing_rejects_invalid_n_runs(
    workspace_path: Path,
) -> None:
    with pytest.raises(ValueError, match="n_runs must be at least 1"):
        benchmark.measure_graph_canonicalization_timing(
            workspace_path, "L_ch0", "FAST_RUN", 0
        )


def test_measure_graph_canonicalization_timing_propagates_build_error(
    monkeypatch: pytest.MonkeyPatch,
    workspace_path: Path,
) -> None:
    def failing_build_log_prob(**kwargs: Any) -> None:
...
def test_run_single_benchmark_propagates_memory_error(
    ...
    monkeypatch.setattr(
        benchmark, "measure_graph_canonicalization_memory", failing_memory
    )
...
def test_print_result_outputs_summary(
    ...
    assert "Graph canonicalization benchmark" in output
...
def test_parse_args_defaults(monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.setattr(sys, "argv", ["run_graph_canonicalization.py"])
...
def test_main_rejects_invalid_arguments(
    ...
    monkeypatch.setattr(
        sys,
        "argv",
        ["run_graph_canonicalization.py", "--workspaces", str(workspace_path), *argv],
    )
...
def test_make_plots_real_png_files_created(
    ...
    assert (tmp_path / "graph_canonicalization_wall_time.png").exists()
    assert (tmp_path / "graph_canonicalization_current_rss_delta.png").exists()
    assert (tmp_path / "graph_canonicalization_peak_rss_delta.png").exists()
...
def test_make_error_result_contains_failed_payload(workspace_path: Path) -> None:
    ...
    assert result["error_message"] == "canonicalization failed"
    assert "RuntimeError: canonicalization failed" in result["traceback"]
...
def test_print_error_result_outputs_summary(
    ...
    result = {
        ...
        "error_message": "canonicalization failed",
    }

    benchmark.print_error_result(result)

    output = capsys.readouterr().out
    assert "Graph canonicalization benchmark FAILED" in output
    assert "workspace.json" in output
    assert "L_ch0" in output
    assert "FAST_RUN" in output
    assert "RuntimeError: canonicalization failed" in output
...
def test_make_plots_skips_when_fewer_than_two_successful_results(
    ...
    failed = valid_result.copy()
    failed["status"] = "failed"

    benchmark.make_plots(results=[valid_result, failed], plot_dir=tmp_path)
...
def test_make_plots_reports_skipped_memory_plots(
    monkeypatch: pytest.MonkeyPatch,
    capsys: pytest.CaptureFixture[str],
    tmp_path: Path,
    valid_result: dict[str, Any],
    ...
    output = capsys.readouterr().out
    assert [call["metric_key"] for call in calls] == ["wall_time_seconds_mean"]
    assert "Skipping current RSS plot: all values are zero." in output
    assert "Skipping peak RSS plot: all values are zero." in output
...
def test_module_main_guard_runs_main_with_patched_context(
    monkeypatch: pytest.MonkeyPatch,
    tmp_path: Path,
    workspace_path: Path,
    valid_result: dict[str, Any],
) -> None:
    output_dir = tmp_path / "module-results"
    ...
    monkeypatch.setattr(
        multiprocessing,
        "get_context",
        lambda method: FakeContext(valid_result),
    )

    runpy.run_module("src.run_graph_canonicalization", run_name="__main__")

    output_path = output_dir / benchmark.DEFAULT_OUTPUT_NAME
    assert output_path.exists()

    with output_path.open() as file:
        payload = json.load(file)

    assert payload["benchmark"] == "graph_canonicalization"
