...
Measures the cost of applying PyTensor/JAX graph optimization rewrites to PyHS3 log-probability graphs.
...
FunctionGraph(...)
    ↓
JAX optimizer rewrites
```

Workspace loading, model creation, and log-probability construction are treated as setup and are intentionally excluded from the timed optimization section.
...
* the optimized graph has one output
* the optimized graph contains apply nodes
* graph inputs are detected
* the number of apply nodes before and after optimization is recorded
* optimization successfully transforms the graph
...
* optimization wall time samples
...
The benchmark builds a fresh PyTensor `FunctionGraph`, applies the JAX optimizer rewrites, and records process-level RSS deltas.

Memory measurements should be interpreted as approximate process-level costs of graph optimization, not exact object-level graph memory.
...
Binned likelihood workspaces may optimize to graphs without apply nodes or require different targets, so they are not used for this benchmark.
...
![Graph Optimization Peak RSS Delta](plots/graph_optimization_simple/graph_optimization_peak_rss_delta.png)
